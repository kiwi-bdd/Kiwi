...
- (void)invocationCapturer:(KWInvocationCapturer *)anInvocationCapturer didCaptureInvocation:(NSInvocation *)anInvocation {
    ...
    if (!(anInvocationCapturer.userInfo)[StubSecondValueKey]) {
        ...
        NSUInteger times = [(anInvocationCapturer.userInfo)[ChangeStubValueAfterTimesKey] integerValue];
...
- (id)stubAndReturn:(id)aValue times:(NSInteger)times afterThatReturn:(id)aSecondValue {
    NSDictionary *userInfo = @{StubValueKey: aValue, ChangeStubValueAfterTimesKey: [NSNumber numberWithInteger:times], StubSecondValueKey: aSecondValue};
    return [KWInvocationCapturer invocationCapturerWithDelegate:self userInfo:userInfo];
}
...
+ (void)stubMessagePattern:(KWMessagePattern *)aMessagePattern andReturn:(id)aValue times:(NSInteger)times afterThatReturn:(id)aSecondValue overrideExisting:(BOOL)override {
    if ([self methodSignatureForSelector:aMessagePattern.selector] == nil) {
        [NSException raise:@"KWStubException" format:@"cannot stub -%@ because no such method exists",
         NSStringFromSelector(aMessagePattern.selector)];
    }
